...
def postprocess(result, img_meta, num_classes=80, rescale=True):
    if result.get('boxes') is None:
        det_bboxes = np.squeeze(result['dets'])
        det_labels = np.squeeze(result['labels'])
    else:
...
def main(args):
    ...
    if backend == 'openvino':
        ...
        if cfg.model.type == 'MaskTextSpotter':
            from openvino_backend import MaskTextSpotterOpenVINO as Model
            extra_args['text_recognition_thr'] = cfg['model'].get(
                'roi_head', {}).get('text_thr', 0.0)
        else:
            from openvino_backend import Detector as Model
...
def parse_args():
    ...
    parser.add_argument(
        ...
        ' "segm", "proposal", "f1" for COCO, and "mAP",'
