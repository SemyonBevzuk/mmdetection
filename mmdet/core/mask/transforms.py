...
def mask2result(det_bboxes,
                ...
                mask_thr_binary=0.5,
                img_size=None):
    masks = det_masks[0] if isinstance(det_masks, list) else det_masks

    # To support openmmlab output with batch=1
    masks_size = len(masks) if isinstance(masks, list) else len(masks.shape)
    if masks_size > 3:
        masks = masks.squeeze()
    ...
    for bbox, label, mask in zip(bboxes, labels, masks):
        ...
        img_masks = F.grid_sample(
            mask.to(dtype=torch.float32)[None, :, :, :],
            grid[None, :, :, :],
            align_corners=False)
